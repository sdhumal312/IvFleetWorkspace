...
public class TripSheetService implements ITripSheetService {
	...
	public List<TripSheetIncomeDto> findAllTripSheetIncome(Long tripSheetID, Integer companyId) throws Exception {
		...
		if((boolean) configuration.get("showLsDestination")) {
			condn = " WHERE T.tripsheet.tripSheetID =:id AND T.companyId =:companyId AND T.markForDelete = 0 GROUP BY T.dispatchLedgerId";
	...
	@Transactional
	public List<TripSheetDto> VehicleTOlistTripSheet(Integer vid) throws Exception {
		CustomUserDetails userDetails = (CustomUserDetails) SecurityContextHolder.getContext().getAuthentication()
				.getPrincipal();
	...
	//Create Day Wise Expense Report Start
		...
		public ValueObject getIVLoadingSheetDataForTrip(ValueObject valueObject) throws Exception {
			...
			try {
				...
				HttpResponse<JsonNode> response = Unirest.post((String)configuration.get("loadingSheetSyncLink"))
		...
		public ValueObject saveDueAmountPaymentByAmount(ValueObject valueObject) throws Exception {
			...
			try {
				...
				if((boolean) CashBookconfig.get("addDueExpenseInCashBook")) {
					...
					try {
						...
						if(valueObject.getShort("billselectionId") == WayBillTypeConstant.WITH_BILL){
							cashbookName = CashBookBalanceStatus.SSTS_CASH_BOOK_NAME;
						}else if(valueObject.getShort("billselectionId") == WayBillTypeConstant.WITHOUT_BILL) {
							cashbookName = CashBookBalanceStatus.JBL_CASH_BOOK_NAME;
						}
						
						CashBookName	bookName =  cashbookNameService.getCashBookByName(cashbookName,userDetails.getCompany_id());
						...
						cashbook.setCASH_BOOK_NO(bookName.getCASHBOOK_NAME());
					 	...
					 	cashbook.setCASH_REFERENCENO("<a href='/showTripSheet.in?tripSheetID=" + trip.getTripSheetID() + "'> TS - " + trip.getTripSheetNumber()+ " </a>");
					 	
					 	Driver driver = driverService.getDriver(trip.getTripFristDriverID(),userDetails);
					 	...
					 	TripExpense expense = tripExpenseService.getTripExpenseByName("DRIVER SALARY", userDetails.getCompany_id());
					 	cashbook.setCASH_NATURE_PAYMENT_ID(expense.getExpenseID());
...
>>>>>>> .merge-right.r2498
								...
								cashbook.setCASH_BOOK_NO(bookName.getCASHBOOK_NAME());
							 	...
							 	TripExpense expense = tripExpenseService.getTripExpenseByName("DRIVER SALARY", companyId);
							 	cashbook.setCASH_NATURE_PAYMENT_ID(expense.getExpenseID());
